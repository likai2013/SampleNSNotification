...
- (void)viewDidLoad
{
    ...
    label.frame = CGRectMake(234, 300, 300, 50);
    label.backgroundColor = [UIColor yellowColor];
    label.textAlignment = UITextAlignmentCenter;
    label.text = @"ここに何行目か表示されます。";
    [self.view addSubview:label];
    
    // 通知センターを取得
    NSNotificationCenter *nc = [NSNotificationCenter defaultCenter];
    [nc addObserver:self selector:@selector(getPickerValueAction:) name:@"PickerValue" object:nil]; // 通知センターに通知要求を登録
}
...
- (void)getPickerValueAction:(NSNotification*)nc
{
    NSString *value = [[nc userInfo] objectForKey:@"KEY"];
